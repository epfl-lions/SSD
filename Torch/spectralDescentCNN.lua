...
cmd:text('Options:')
cmd:option('--optimization','RMSspectral','Choices of SGD, RMSprop, and RMSspectral')
...
cmd:option('--channelSize', '{64}', 'Number of output channels for each convolution layer.')
...
cmd:option('--batchSize', 1000, 'number of examples per batch')
...
cmd:option('--hiddenSize', '{300}', 'size of the dense hidden layers after the convolution')
...
--[[optimizer 2]]--
--function opfunc(model,opt)
  function opfunc(model)
        model:updateGradParameters(opt.momentum)
        x,g=model:getParameters()
        return x,g
     end
train = dp.Optimizer{
   ...
   callback = function(model, report) -- called every batch
      if opt.optimization == 'SGD' then
          x,g=opfunc(model)
          x:add(-opt.learningRate,g)
      elseif opt.optimization == 'RMSprop' then
          x,g=opfunc(model)
          config = config or {}
          g=sd.RMSpropsteps(g, config, model)
          x:add(-opt.learningRate,g)
      elseif opt.optimization == 'RMSspectral' then
          x,g=opfunc(model)
          config = config or {}
          config.k=30
          g=sd.RMSspectralSplitModelSteps(g, config, model)
          x:add(-opt.learningRate,g)
      end
